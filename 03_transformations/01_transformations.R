...
# Inversion
google_analytics_summary_long_tbl %>%
    plot_time_series(date, log1p(value) %>% expm1(), name)
...
subscribers_daily_tbl %>%
    plot_time_series(optin_time,
                     box_cox_vec(optins + 1,
                                 lambda = -.18) %>% box_cox_inv_vec(lambda = -.18))


# ** Google Analytics (Groups)
google_analytics_summary_long_tbl %>%
    plot_time_series(
        date, box_cox_vec(value)
    )
...
google_analytics_summary_long_tbl %>%
    ...
    map2(.y = 1:3, .f = function(df, idx) {
        ...
        df %>%
            mutate(
                value_trans_inv = box_cox_inv_vec(value_trans, lambda = lambda)
            )
    }) %>%
    bind_rows() %>%
    group_by(name) %>%
    plot_time_series(date, value_trans_inv, name)
...
google_analytics_summary_long_tbl %>%
    ...
    group_by(names) %>%
    plot_time_series(date, value, names_2)


# * LOESS smoother ----
google_analytics_summary_long_tbl %>%
    mutate(
        value_roll_smooth = smooth_vec(
            value,
            period = 24 * 7
        )
    ) %>%
    ...
    rename(names = `name...2`, names_2 = `name...3`) %>%
    group_by(names) %>%
    plot_time_series(date, value, names_2, .smooth = F)
...
google_analytics_summary_tbl %>%
    mutate(rolling_cor_pageviews_organic = rolling_cor_24_7(pageViews,
                                                            organicSearches)
    ) %>%
...
transactions_tbl %>%
    mutate(mavg_8_weeks = slidify_vec(revenue,
                                      .f = ~ mean(.x, na.rm = T),
                                      .period = 8, 
                                      .align = "right")
    ) %>%
    ...
    fill(mavg_8_weeks, .direction = "down") %>%
...
subscribers_daily_tbl %>%
    ...
    plot_time_series(optin_time, log1p(value), .color_var = name)
...
subscribers_daily_tbl %>%
    ...
    mutate(optins_diff_1 = diff_vec(total_optins, lag = 1)) %>%
...
google_analytics_summary_diff <- tbl <- google_analytics_summary_tbl %>%
    mutate(across(pageViews:sessions, .fns = diff_vec))

google_analytics_summary_diff %>%
    mutate(dateHour = ymd_h(dateHour)) %>%
    pivot_longer(-dateHour) %>%
    plot_time_series(dateHour, value, name, .smooth = F)
...
subscribers_daily_tbl %>%
    mutate(cos14_k1 = fourier_vec(optin_time, period = 14, K = 1)) %>%
...
data_transformed_tbl <- subscribers_daily_tbl %>%
    mutate(
        optins_trans = log_interval_vec(
            optins,
            limit_lower = limit_lower,
            offset = offset
        )
    ) %>%
    tk_augment_fourier(optin_time,
...
model_forumla <- as.formula(
    optins_trans ~ as.numeric(optin_time) + 
...
model_fit_lm <- lm(model_forumla, data = data_transformed_tbl)
...
# * Predict ----
predictions <- predict(model_fit_lm, future_tbl) %>% as.vector()
...
forecast_tbl %>%
    pivot_longer(cols = c(optins_trans, conf_lo, conf_hi)) %>%
    mutate(value_inv = log_interval_inv_vec(
        x = value, 
        limit_lower = limit_lower,
        limit_upper = limit_upper,
        offset = offset
    )) %>%
    plot_time_series(optin_time, value_inv, name, .smooth = F)
